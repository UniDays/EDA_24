...
namespace EDCHOST22
{
    public partial class Tracker : Form
    {
        ...
        public void ShowMessage(byte[] M)
        {
            // A,B车的总分数
            ...
            // A,B车的得分明细
            label_AMessage.Text =$"获得外卖资数　　{game.CarA.mPkgCount}\n";
            label_BMessage.Text =$"获得外卖资数      {game.CarB.mPkgCount}\n";
            ...
            //场上外卖数量
            label_PackageCount = $"场上外卖数量 { game.PackageOnStage().Count}\n";

            //外卖具体信息
            /*label_PackageLocation = $"外卖信息：\n";
            foreach(Package pkg in game.PackageOnStage())
            {
                label_PackageLocation += $"{pkg.Departure.x pkg.Departure.y pkg.Destination.x pkg.Destination.y}";
            }*/
        }
